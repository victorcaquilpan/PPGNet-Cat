...
* **Demo**: There is a demo available (**demo.ipynb**) to run the evaluation model over a set of 20 images randomly selected from our test subset, creating the embedding of each image. The output of this script generates a **gif** animation with each one of the sample images and their estimated location in a two-dimension space, after using a UMAP algorithm to reduce the 2560 embedding size to a size of 2. Below there is an example of this process, where we can see how our model allocates images of the same entity close to each other. Here, each color represent a different entity.
